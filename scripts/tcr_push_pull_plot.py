...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(description="Plot TCR push/pull benchmark summary CSV.")
    p.add_argument("--summary-csv", type=Path, default=Path("tcr-push-pull-out/summary.csv"))
    p.add_argument("--out", type=Path, default=Path("tcr-push-pull-out/push_plot.png"))
    p.add_argument(
        "--metric",
        type=str,
        default="push_sec_median",
        help="CSV column to plot (e.g. push_sec_median, pull_sec_median, push_sec_p90)",
    )
    ...
    ax.invert_yaxis()
    ax.set_xlabel(f"{args.metric} (sec)")
    ax.set_title("GitHub Runner → TCR push/pull benchmark")

    for i, (v, sr) in enumerate(zip(values, success)):
        ax.text(v, i, f"  {v:.2f}s, ok={sr*100:.0f}%", va="center", ha="left", fontsize=9)
